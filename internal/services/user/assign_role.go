...
func (s *service) AssignRole(ctx context.Context, command commands.AssignRole) error {
	const fn = "services.user.service.AssignRole"
	...
	err = s.userRepo.AddRoleTx(ctx, tx, command)
	if err != nil {
		const msg = "failed to add role to user"
	...
	log.Info("successfully added new role to user", slog.Int64("user_id", command.UserID), slog.String("new_role", command.Role))
